...
Наблюдения: потоковая версия не ускоряет CPU-bound из-за GIL, а процессная обычно заметно быстрее синхронной при наличии нескольких ядер. Все реализации возвращают одинаковую сумму.
...
Параллелизм ограничен --limit, результаты пишутся в results.txt. Если установлена tqdm, отображается прогресс-бар.
